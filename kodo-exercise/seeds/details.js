...
// const { createAdmin } = require('../seeds/admin')
// const { seedSiteSettings } = require('../seeds/sitesettings')
// const { seedMaintenanceSettings } = require('../seeds/maintenancesettings')
...
//     seedSettings
// };
const MaintenanceSettings = require('../models/details')
const seedMaintenanceSettings = async () => {
    ...
    for (let val of check_item) {
        let check_ex = await MaintenanceSettings.findOne({ name: val.name });
        if (!check_ex) {
            await new MaintenanceSettings(val).save();
        }
    }
    return true;
}
